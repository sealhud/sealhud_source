...
export default class PositionBar extends React.Component<IProps, {}> {
  ...
  @observable accessor playersAhead = INVALID;

  @observable accessor playersBehind = INVALID;
  ...
  @observable accessor playerPosition = INVALID;

  @observable accessor positionBarCount = 15;

  @observable accessor entryWidth = 148;

  @observable accessor classColorUpdate: any;
  ...
  private sortByLapDistance = (a: IDriverInfo, b: IDriverInfo) => {
    return (
      b.gapToPlayer - a.gapToPlayer ||
      (a.lapDistance === b.lapDistance &&
        (a.isUser || b.isUser) &&
        ((a.aheadOrBehind.wasAhead === b.aheadOrBehind.wasAhead
          ? 0
          : a.aheadOrBehind.wasAhead
          ? -1
          : 1) ||
          (b.aheadOrBehind.wasBehind === a.aheadOrBehind.wasBehind
            ? 0
            : b.aheadOrBehind.wasBehind
            ? -1
            : 1))) ||
  ...
  private update = () => {
    ...
    if (
      (highPerformanceMode && nowCheck - this.lastCheck >= 33) ||
      (lowPerformanceMode && nowCheck - this.lastCheck >= 266) ||
      (!lowPerformanceMode &&
        !highPerformanceMode &&
      ...
      this.actualRoundsLeft =
        eRoundsLeft > -1 ? eRoundsLeft : getRoundsLeft(this.lapTimeBestSelf);
      ...
      this.sessionTimeRemaining = showAllMode
        ? 1
        : r3e.data.SessionTimeRemaining;
      ...
      if (this.props.relative) {
        ...
        driverData = uniq(driverData.slice(start, end));
        /*driverData = uniq(
					driverData
  ...
  private wasAheadOrBehind = (driverData: IDriverInfo[], searchId: number) => {
    if (searchId === this.currentSlotId) {
      return 0;
    }
    for (let i = 0; i < driverData.length; i++) {
      if (driverData[i].id === searchId) {
        if (driverData[i].aheadOrBehind.wasAhead) {
          return 1;
        }
        if (driverData[i].aheadOrBehind.wasBehind) {
          return -1;
        }
      }
      return 0;
  ...
  private getAheadOrBehind = (
    ...
    for (let i = 0; i < driverData.length; i++) {
      if (driverData[i].id === searchId) {
        ...
        if (lapDist > playersDist) {
          // || place < r3e.data.Position) {
          iAhead = true;
          iBehind = false;
          wAhead = true;
          wBehind = false;
        } else if (lapDist < playersDist) {
          // || place > r3e.data.Position) {
          iAhead = false;
          iBehind = true;
          wAhead = false;
          wBehind = true;
        } else if (lapDist === playersDist && driver.aheadOrBehind.wasAhead) {
          // || place < r3e.data.Position) {
          iAhead = true;
          iBehind = false;
          wAhead = true;
          wBehind = false;
        } else if (lapDist === playersDist && driver.aheadOrBehind.wasBehind) {
          // || place > r3e.data.Position) {
          iAhead = false;
          iBehind = true;
          wAhead = false;
  ...
  private formatDriverData = (driver: IDriverData, i: number): IDriverInfo => {
    ...
    const aheadOrBehind = isUser
      ? 0
      : driver.LapDistance === this.lapDistance
      ? this.drivers.length > i &&
        this.drivers[i] !== undefined &&
        this.drivers[i].id === driver.DriverInfo.SlotId
    ...
    const driverData = {
      ...
      aheadOrBehind: isUser
        ...
        : this.getAheadOrBehind(
            this.drivers,
            driver.DriverInfo.SlotId,
            gapPlayer
          ),
  ...
  private calculateDiffsQualify(drivers: IDriverInfo[]) {
    const userBestSector =
      this.bestSelfSector3 !== INVALID ? this.bestSelfSector3 : 0;
  ...
  private calculateDiffsRace(drivers: IDriverInfo[]) {
    ...
    driversInfront.reverse().forEach((driver) => {
      let gotLapped = false;
      if (driver.lapDiff === 1 && driver.lapDistance > this.lapDistance) {
        gotLapped = true;
        driver.diff = `-${Math.abs(driver.lapDiff)} ${_("lap")}`;
      }
      if (driver.lapDiff > 1) {
        gotLapped = true;
        if (driver.lapDistance > this.lapDistance) {
  ...
  private calculateDiffsRaceRelative(drivers: IDriverInfo[]) {
    ...
    drivers.forEach((driver) => {
      ...
      const bestLapLeader =
        r3e.data.LapTimeBestLeader > -1 ? r3e.data.LapTimeBestLeader : 0;

      const bestLapOpponent =
        driver.meta && driver.meta.SectorTimeBestSelf.Sector3 > 0
      ...
      const avgSpeed = (avgLapSpeed + avgLapSpeedOpponent) / 2;
  ...
  render() {
    ...
    if (this.playerCount === 1 && this.props.relative && !showAllMode) {
      return null;
    }
                    ...
                    ? this.props.settings.subSettings.currentPosition.enabled
                      ? this.props.settings.subSettings.sessionLaps.enabled
                        ? this.actualRoundsLeft > 99 ||
                          Math.ceil(
                            this.sessionTimeDuration / this.bestLapSelf
                          ) >= 100
                          ? "490px"
                          : "480px"
                        : this.actualRoundsLeft > 99 ||
...
export class PositionEntry extends React.Component<IEntryProps, {}> {
  ...
  render() {
    ...
    if (sessionType === 2 && sessionPhase === 1) {
      return null;
    }
        ...
        >
          ...
          {!this.props.relative &&
            ...
            ((this.props.settings.subSettings.showLastLaps.enabled &&
              ...
              nowCheck <= playerLapInfo[player.id][2] && (
                <div
                  className={classNames("diff mono")}
                  style={{
                    color: !(

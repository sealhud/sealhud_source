...
export default class Graphs extends React.Component<IProps, {}> {
	@observable accessor selectedkeys: string[] = [];

	@observable accessor editing = false;

	@observable accessor statTrackingCount = 0;
	...
	private getAllPaths() {
		...
		const traverse = (data: Record<string, any>, subPath = '', depth: number) => {
			if (depth > 1) return;
	...
	render() {
		...
		return (
			<div
				className={classNames('graphs', this.props.className, {
